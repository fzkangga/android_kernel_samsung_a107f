...
static void vibrator_enable(unsigned int dur, unsigned int activate)
{
	...
	pr_info(VIB_TAG "cancel hrtimer, cust:%dms, value:%u, shutdown:%d\n",
...
static ssize_t enable_show(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	struct mt_vibr *ddata = g_mt_vib;
	struct hrtimer *timer = &ddata->vibr_timer;
	int remaining = 0;
...
static int vib_probe(struct platform_device *pdev)
{
	...
	vibr->to_class = class_create(THIS_MODULE, "timed_output");
	if (IS_ERR(vibr->to_class)) {
		ret = PTR_ERR(vibr->to_class);
	}
	vibr->to_dev = device_create(vibr->to_class, NULL,
		MKDEV(0, 0), vibr, "vibrator");
	if (IS_ERR(vibr->to_dev)) {
		ret = PTR_ERR(vibr->to_dev);
	}

	ret = sysfs_create_group(&vibr->to_dev->kobj, &timed_output_attr_group);
	if (ret) {
		ret = -ENODEV;
