...
{
	const char *soc_name;
	unsigned int hw_code = __chip_hw_code() & 0xFFFF;
	unsigned int hw_subcode = __chip_hw_subcode() & 0xFFFF;

	unsigned int chip_id = hw_code << 16 | hw_subcode;
...
static char * uniqueid_get(void)
{
	...
	strncpy(dest,s1,16); //serialno length=40
	dest[16]='\0';
	s1 = dest;
