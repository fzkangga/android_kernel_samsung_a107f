...
unsigned int get_mr5_dram_mode_reg(void)
{
	unsigned int res;
	unsigned int mr5_cur;
	void __iomem *dramc_ao_chx_base = DRAMC_AO_CHA_BASE_ADDR;
	void __iomem *dramc_nao_chx_base = DRAMC_NAO_CHA_BASE_ADDR;

	if (!dramc_ao_chx_base || !dramc_nao_chx_base)
		return 0;

	res = read_dram_mode_reg(5, &mr5_cur, dramc_ao_chx_base, dramc_nao_chx_base);
	if (res != TX_DONE)
		return 0;

	return (mr5_cur & 0xFF);
}
EXPORT_SYMBOL(get_mr5_dram_mode_reg);
