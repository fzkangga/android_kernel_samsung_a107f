/* to Support headset detect function for sec jack extension. KSND  */
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	pr_warn("%s: [accdet] adc_val %d\n", __func__, jackdet->adc_val);
...
void create_jack_ex_devices(struct jack_device_priv *pPriv_dat)
{
	...
	earjack = device_create(audio_class, NULL,
				MKDEV(0, index), NULL, "earjack");
