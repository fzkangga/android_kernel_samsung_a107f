...
//Type Bug431177 + , chenshengqian.wt, modify, 2019.03.23, fixed regulator val setting issue
	...
	enum IMGSENSOR_HW_PIN_STATE pin_state)
{
	...
	//	regulator_control[(reg_type_offset + pin - IMGSENSOR_HW_PIN_AVDD)].pregulator_type,
	//	regulator_status[(reg_type_offset+ pin - IMGSENSOR_HW_PIN_AVDD)]);
...
static void check_for_regulator_get(struct REGULATOR *preg, struct device *pdevice, int index)
{
	...
	if(regulator_status[index]==0)
	{
		...
		preg->pregulator[index] = regulator_get(pdevice,
