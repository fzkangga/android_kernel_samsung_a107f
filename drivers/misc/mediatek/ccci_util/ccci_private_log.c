	...
	unsigned int id, unsigned int size)
{
	...
	virt = phys_to_virt((phys_addr_t)cccimdee_reserved_phy_addr);
	cccimdee_reserved_phy_addr += size;
	pr_notice("[ccci]alloc ee dump DT reserve memory:zone %d,id:%d, size:0x%x, virt:0x%llx, phy:0x%llx\n",
		zone, id, size, (unsigned long long)virt,
