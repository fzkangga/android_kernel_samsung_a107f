...
{
        strlcpy(un_buf, line, sizeof(un_buf));
	return 1;
}     
		...
		char *buf)
{
	...
	if (strncmp(un_buf, buf, strlen(un_buf)) != 0){
		pr_info("%s / MMC UN mismatch\n", __func__);
		BUG_ON(1);
	}
	else
		return n;
...
//Bug 432506, donghaibo2@wt, 2019.3.14, add EMMC flash_name & vendor_name start
	...
	char *buf)
{
	...
	switch (card->cid.manfid) {
		...
		case 0x15:
			vendor_name = "Samsung";
			if (strncmp(card->cid.prod_name, "QD63MB", strlen("QD63MB")) == 0)
...
//Bug 432506, donghaibo2@wt, 2019.3.14, add EMMC flash_name & vendor_name end
...
static struct attribute *mmc_std_attrs[] = {
	...
	&dev_attr_ocr.attr,
	&dev_attr_dsr.attr,
	&dev_attr_flash_name.attr,  //Bug 432506, donghaibo2@wt, 2019.3.14, add
	&dev_attr_vendor.attr,  //Bug 432506, donghaibo2@wt, 2019.3.14, add
