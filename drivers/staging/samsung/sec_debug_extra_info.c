...
void sec_debug_init_extra_info(struct sec_debug_shared_info *sec_debug_info,
				 phys_addr_t sec_extra_info_size)
{
	if (sec_debug_info) {
		sec_debug_extra_info = &sec_debug_info->sec_debug_extra_info;
		sec_debug_extra_info_buf = (char *)sec_debug_info
						 + sec_extra_info_size - SZ_1K;
	}
...
void sec_debug_store_extra_info(void)
{
  ...
  ptr += snprintf(ptr, SZ_1K, "\"%s\":\"%s\"", sec_debug_extra_info->item[INFO_KTIME].key,
      sec_debug_extra_info->item[INFO_KTIME].val);
		

	for (i = 1; i < INFO_MAX; i++) {
		if (ptr + strlen(sec_debug_extra_info->item[i].key)
			 + strlen(sec_debug_extra_info->item[i].val)
...
extern uint32_t lmk_count;
extern u64 last_lmkclock;
/******************************************************************************
 * sec_debug_set_extra_info_lmk
******************************************************************************/

void sec_debug_set_extra_info_lmk(void)
{
	u64 ts_nsec;

	ts_nsec = last_lmkclock;
	do_div(ts_nsec, 1000000000);
	sec_debug_set_extra_info(INFO_LMK, "%lu:%d", (unsigned long)ts_nsec,
					(unsigned int)lmk_count);
}
#else
#define sec_debug_set_extra_info_lmk()		do { } while(0)
#endif
