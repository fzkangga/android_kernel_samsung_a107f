...
atomic_long_t mm_heap_allocated;
atomic_long_t mm_sec_heap_allocated;

void update_mtk_ion_heap_size(unsigned long len, enum ion_heap_type type, bool add)
{
	atomic_long_t *allocated;

	if (type == ION_HEAP_TYPE_MULTIMEDIA)
		allocated = &mm_heap_allocated;
	else if(type == ION_HEAP_TYPE_MULTIMEDIA_SEC)
		allocated = &mm_sec_heap_allocated;
	else
		return;

	if (add)
		atomic_long_add(len, allocated);
	else
		atomic_long_sub(len, allocated);
}

void show_ion_mtk_mm_heap_size(struct seq_file *s)
{
	unsigned long mm_heap_byte = 0, mm_sec_heap_byte = 0;
	
	mm_heap_byte = (unsigned long)atomic_long_read(&mm_heap_allocated);
	mm_sec_heap_byte = (unsigned long)atomic_long_read(&mm_sec_heap_allocated);

	if (s) {
		seq_printf(s, "MMHeap:         %8lu kB\n", mm_heap_byte >> 10);
		seq_printf(s, "MMSecHeap:      %8lu kB\n", mm_sec_heap_byte >> 10);
	} else {
		pr_cont("MMHeap:%lukB MMSecHeap:%lukB ", mm_heap_byte >> 10, mm_sec_heap_byte >> 10);
	}
}

static int ion_mtk_mm_heap_size_notifier(struct notifier_block *nb,
					 unsigned long action, void *data)
{
	show_ion_mtk_mm_heap_size((struct seq_file *)data);
	return 0;
}

static struct notifier_block ion_mtk_mm_heap_nb = {
	.notifier_call = ion_mtk_mm_heap_size_notifier,
};
...
static int __init ion_init(void)
{
	...
	show_mem_extra_notifier_register(&ion_mtk_mm_heap_nb);
	return 0;
}
