					    ...
					    unsigned long flags)
{
	...
	update_mtk_ion_heap_size(len, heap->type, 1);
...
static void _ion_buffer_destroy(struct kref *kref)
{
	...
	update_mtk_ion_heap_size(buffer->size, heap->type, 0);
			     ...
			     unsigned int flags)
{
	...
	if (IS_ERR(buffer)) {
		IONMSG("%s: len %zu align %zu heap_id_mask %#x flags %x ret %ld\n",
		       __func__, len, align, heap_id_mask, flags, PTR_ERR(buffer));
