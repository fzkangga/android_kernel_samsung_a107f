...
#define WT_ADD_SAR_HARDWARE_INFO 1  //bug 390577,gjx.wt,add,20180905,add hardware info
#define SAR_PINCTRL_EN 0
...
//+bug 390577,gjx.wt,add,20180905,add hardware info
#if WT_ADD_SAR_HARDWARE_INFO
#include <linux/hardware_info.h>  //Bug 342730,gaojingxuan.wt,20180228,add hardware info
#endif
//-bug 390577,gjx.wt,add,20180905,add hardware info
//+Bug 354802,gaojingxuan.wt,20180514,add,calibrate sar when usb status change
...
static int abov_fw_update(bool force)
{
	...
	strlcpy(fw_name, this->fw_name, sizeof(fw_name)); //bug 444368,20190509,modify
	...
	if (fw_upgrade) {
		for (update_loop = 0; update_loop < 10; update_loop++) {
			...
			if (rc < 0)
                ...
                this->init(this);  //Bug 351219,gaojingxuan.wt,add,init after upgrade
				break;
            }
			msleep(400);
		}
...
//+Bug 350219 ,gaojingxuan.wt,20180411,add sar force update
 ...
 static int read_irq_status(psxT316XX_t this)
{
	...
	if (this) {
		if (read_register(this,SXT316_IRQSTAT_REG,&data) == 0) {
		    printk("sxT316 read 0x00 = 0x%x\n", (data & 0x00FF));   //Bug 348446,gaojingxuan.wt,modify,open sar log/close ch1
            return (data & 0x00FF);
        }
	}
	return 0;
...
static int enable_sar = -1; //gjx.wt,20190130,for sar suspend into sleep mode
...
//bug 425261,20190107,gjx.wt,ATO close sar MP open sar,start
#ifdef FACTORY_VERSION
   write_register(this,SXT316_CTRL_MODE,2);
#elif defined(USER_VERSION)
   write_register(this,SXT316_CTRL_MODE,0);
   enable_sar=1;
#else
   write_register(this,SXT316_CTRL_MODE,2);
   enable_sar=0; //gjx.wt,20190130,for sar suspend into sleep mode
#endif
//bug 425261,20190107,gjx.wt,ATO close sar MP open sar,end
#ifdef CONFIG_SAR_SENSOR_CHN_SELET
    write_register(this,SXT316_CH_REG,0);
#else
   write_register(this,SXT316_CH_REG,0x05); //Bug 348446,gaojingxuan.wt,modify,enable ch_ref and ch0
#endif
    //write_register(this,SXT316_GAIN,2); //Bug 351389,gaojingxuan.wt,modify,delete gain in driver
...
static void PRC_touchProcess(psxT316XX_t this)
{
  ...
  if (this && (pDevice = this->pDevice))
  {
    ...
    if (unlikely( (buttons==NULL) || (input==NULL) )) {
	  ...
	  near_far= (status & 0x1);
	  printk("sxT316 prc near_far %s\n",(near_far ? "near" : "far"));  //Bug 348446,gaojingxuan.wt,modify,open sar log/close ch1
...
printk("sar sxT316_irq_gpio = %d",sxT316_irq_gpio);
		...
		const char *buf, size_t count)
{
	...
	ret=write_register(this,SXT316_CH_REG,(val|0x04));   //gjx.wt,modify,20190410,enable ch_ref
	...
	if (client) {
		for (i = 0; i < 3; i++) {
			...
			if(i == 1) {
				...
				//msleep(100);   //Bug 353989 ,gaojingxuan.wt,20180503,delete delay,i2C read at least 100ms form sensor VDD on
			}
		}
...
#if SAR_PINCTRL_EN
static int sar_pinctrl_init(struct i2c_client *client)
{
    int ret = 0;

    abov_sar_ptr->pinctrl = devm_pinctrl_get(&client->dev);
    if (IS_ERR_OR_NULL(abov_sar_ptr->pinctrl)) {
        printk("Failed to get pinctrl, please check dts");
        ret = PTR_ERR(abov_sar_ptr->pinctrl);
        goto err_pinctrl_get;
    }

    abov_sar_ptr->pin_default = pinctrl_lookup_state(abov_sar_ptr->pinctrl, "pin_default");
    if (IS_ERR_OR_NULL(abov_sar_ptr->pin_default)) {
        printk("Pin state[pin_default] not found");
        ret = PTR_ERR(abov_sar_ptr->pin_default);
        goto err_pinctrl_lookup;
    }

    abov_sar_ptr->pin_state_eint_as_int = pinctrl_lookup_state(abov_sar_ptr->pinctrl, "pin_state_eint_as_int");
    if (IS_ERR_OR_NULL(abov_sar_ptr->pin_state_eint_as_int)) {
        printk("Pin state[pin_state_eint_as_int] not found");
        ret = PTR_ERR(abov_sar_ptr->pin_state_eint_as_int);
        goto err_pinctrl_lookup;
    }

    return 0;
err_pinctrl_lookup:
    if (abov_sar_ptr->pinctrl) {
        devm_pinctrl_put(abov_sar_ptr->pinctrl);
    }
err_pinctrl_get:
    abov_sar_ptr->pinctrl = NULL;
    abov_sar_ptr->pin_default = NULL;
    abov_sar_ptr->pin_state_eint_as_int = NULL;
    return ret;
}
/*
static int sar_pinctrl_select_default(void)
{
    int ret = 0;

    if (abov_sar_ptr->pinctrl && abov_sar_ptr->pin_default) {
        ret = pinctrl_select_state(abov_sar_ptr->pinctrl, abov_sar_ptr->pin_default);
        if (ret < 0) {
            printk("Set pin_default state error:%d", ret);
        }
    }
...
*/
static int sar_pinctrl_select_eint(void)
{
    int ret = 0;

    if (abov_sar_ptr->pinctrl && abov_sar_ptr->pin_state_eint_as_int) {
        ret = pinctrl_select_state(abov_sar_ptr->pinctrl, abov_sar_ptr->pin_state_eint_as_int);
        if (ret < 0) {
            printk("Set pin_state_eint_as_int state error:%d", ret);
        }
    }
...
static int sxT316_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    int i;
#if  SAR_USB_CHARGED
	struct power_supply *psy = NULL;    //Bug 354802,gaojingxuan.wt,20180514,add,calibrate sar when usb status change
#endif
//+bug 390577,gjx.wt,add,20180905,add hardware info
#if WT_ADD_SAR_HARDWARE_INFO
    u8 fw_version = 0;
...
//-Bug 350219 ,gaojingxuan.wt,20180411,add sar force update
	...
	// client->addr=0x20;
	err = of_get_sxT316_platform_data(&client->dev);
printk("gjx err = %d",err);
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_READ_WORD_DATA))
		return -EIO;
	this = kzalloc(sizeof(sxT316XX_t), GFP_KERNEL); /* create memory for main struct */
    abov_sar_ptr = this;
#if SAR_PINCTRL_EN
    sar_pinctrl_init(client);
    sar_pinctrl_select_eint();
#endif
	if (this)
	{
...
#if WT_ADD_SAR_HARDWARE_INFO
        read_register(this, ABOV_VERSION_REG, &fw_version);
        snprintf(firmware_ver,HARDWARE_MAX_ITEM_LONGTH,"A96T346HW,FW:0x%x",fw_version);
