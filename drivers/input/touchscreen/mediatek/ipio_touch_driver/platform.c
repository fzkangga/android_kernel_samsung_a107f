...
#endif
{
	...
	ilitek_proc_remove();
	sec_cmd_exit(&ipd->sec, SEC_CLASS_DEVT_TSP);
	return 0;
}
...
static void get_fw_ver_bin(void *device_data)
{
	...
	if (protocol->mid >= 0x3) {
	    snprintf(buff, sizeof(buff) ,"ILI9881H,VND:%s,FW:%d.%d.%d.%d",ven_name[name_modue],\
	            (core_firmware->new_fw_cb&0xff000000)>>24,( core_firmware->new_fw_cb&0xff0000)>>16, (core_firmware->new_fw_cb&0xff00)>>8,(core_firmware->new_fw_cb&0xff));
	} else {
	    snprintf(buff, sizeof(buff) ,"ILI9881H,VND:%s,FW:%d.%d.%d",ven_name[name_modue],\
	           (core_firmware->new_fw_cb&0xff000000)>>24,( core_firmware->new_fw_cb&0xff0000)>>16, (core_firmware->new_fw_cb&0xff00)>>8);
	}
...
#endif
{
	...
	ret = sec_cmd_init(&ipd->sec, ft_commands,ARRAY_SIZE(ft_commands), SEC_CLASS_DEVT_TSP);
	if (ret < 0) {
