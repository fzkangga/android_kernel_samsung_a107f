...
#if defined(SEC_TSP_FACTORY_TEST) || defined(HX_AUTO_UPDATE_FW)
...
#ifdef LCD_TP_RESUME_WORK
extern void himax_resume_workqueue(struct work_struct *work);
#endif
...
struct proc_dir_entry *himax_proc_self_raw_file;
#endif
									...
									size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if ((val & HX_INSPECT_EFILE) == HX_INSPECT_EFILE) {
			ret += snprintf(temp_buf + ret, len - ret, "  Get criteria File Fail\n");
			goto END_FUNC;
		} else {
			ret += snprintf(temp_buf + ret, len - ret, "  Get criteria File OK\n");
		}
		if ((val & HX_INSPECT_EGETRAW) == HX_INSPECT_EGETRAW) {
			ret += snprintf(temp_buf + ret, len - ret, "  Get raw data Fail\n");
			goto END_FUNC;
		} else {
			ret += snprintf(temp_buf + ret, len - ret, "  Get raw data OK\n");
		}
		if (g_inspt_crtra_flag[IDX_RAWMIN] == 1 && g_inspt_crtra_flag[IDX_RAWMAX] == 1) {
			if ((val & HX_INSPECT_ERAW) == HX_INSPECT_ERAW)
				ret += snprintf(temp_buf + ret, len - ret, "  Check Raw data Fail\n");
			else
				ret += snprintf(temp_buf + ret, len - ret, "  Check Raw data OK\n");
		}
		if (g_inspt_crtra_flag[IDX_NOISEMAX] == 1) {
			if ((val & HX_INSPECT_ENOISE) == HX_INSPECT_ENOISE)
				ret += snprintf(temp_buf + ret, len - ret, "  Check Noise Fail\n");
			else
				ret += snprintf(temp_buf + ret, len - ret, "  Check Noise OK\n");
		}
		if (g_inspt_crtra_flag[IDX_OPENMIN] == 1 && g_inspt_crtra_flag[IDX_OPENMAX] == 1) {
			if ((val & HX_INSPECT_EOPEN) == HX_INSPECT_EOPEN)
				ret += snprintf(temp_buf + ret, len - ret, "  Check Sensor open Fail\n");
			else
				ret += snprintf(temp_buf + ret, len - ret, "  Check Sensor open OK\n");
		}
		if (g_inspt_crtra_flag[IDX_M_OPENMIN] == 1 && g_inspt_crtra_flag[IDX_M_OPENMAX] == 1) {
			if ((val & HX_INSPECT_EMOPEN) == HX_INSPECT_EMOPEN)
				ret += snprintf(temp_buf + ret, len - ret, "  Check Sensor micro open Fail\n");
			else
				ret += snprintf(temp_buf + ret, len - ret, "  Check Sensor micro open OK\n");
		}
		if (g_inspt_crtra_flag[IDX_SHORTMIN] == 1 && g_inspt_crtra_flag[IDX_SHORTMAX] == 1) {
			if ((val & HX_INSPECT_ESHORT) == HX_INSPECT_ESHORT)
				ret += snprintf(temp_buf + ret, len - ret, "  Check Sensor short Fail\n");
			else
				ret += snprintf(temp_buf + ret, len - ret, "  Check Sensor short OK\n");
		}
		if ((g_inspt_crtra_flag[IDX_GAP_HOR_RAWMIN] == 1 && g_inspt_crtra_flag[IDX_GAP_HOR_RAWMAX] == 1) ||
			...
			if ((val & HX_INSPECT_EGAP_RAW) == HX_INSPECT_EGAP_RAW)
				ret += snprintf(temp_buf + ret, len - ret, "  Check Raw Data GAP Fail \n");
			else
				ret += snprintf(temp_buf + ret, len - ret, "  Check Raw Data GAP OK \n");
		}
		if (g_inspt_crtra_flag[IDX_ACT_IDLE_RAWDATA_MIN] == 1 && g_inspt_crtra_flag[IDX_ACT_IDLE_RAWDATA_MAX] == 1) {
			if ((val & HX_INSPECT_EACT_IDLE_RAW) == HX_INSPECT_EACT_IDLE_RAW)
				ret += snprintf(temp_buf + ret, len - ret, "  Check ACT_IDLE RAW Fail\n");
			else
				ret += snprintf(temp_buf + ret, len - ret, "  Check ACT_IDLE RAW OK\n");
		}
		if (g_inspt_crtra_flag[IDX_ACT_IDLE_NOISE_MIN] == 1 && g_inspt_crtra_flag[IDX_ACT_IDLE_NOISE_MAX] == 1) {
			if ((val & HX_INSPECT_EACT_IDLE_NOISE) == HX_INSPECT_EACT_IDLE_NOISE)
				ret += snprintf(temp_buf + ret, len - ret, "  Check  ACT_IDLE NOISE Fail\n");
			else
				ret += snprintf(temp_buf + ret, len - ret, "  Check  ACT_IDLE NOISE OK\n");
		}
		if (g_inspt_crtra_flag[IDX_LPWUG_RAWDATA_MIN] == 1 && g_inspt_crtra_flag[IDX_LPWUG_RAWDATA_MAX] == 1) {
			if ((val & HX_INSPECT_ELPWUG_RAW) == HX_INSPECT_ELPWUG_RAW)
				ret += snprintf(temp_buf + ret, len - ret, "  Check LPWUG RAW  Fail\n");
			else
				ret += snprintf(temp_buf + ret, len - ret, "  Check LPWUG RAW  OK\n");
		}
		if (g_inspt_crtra_flag[IDX_LPWUG_NOISE_MIN] == 1 && g_inspt_crtra_flag[IDX_LPWUG_NOISE_MAX] == 1) {
			if ((val & HX_INSPECT_ELPWUG_NOISE) == HX_INSPECT_ELPWUG_NOISE)
				ret += snprintf(temp_buf + ret, len - ret, "  Check  LPWUG NOISE Fail\n");
			else
				ret += snprintf(temp_buf + ret, len - ret, "  Check  LPWUG NOISE OK\n");
		}
		if (g_inspt_crtra_flag[IDX_LPWUG_IDLE_RAWDATA_MIN] == 1 && g_inspt_crtra_flag[IDX_LPWUG_IDLE_RAWDATA_MAX] == 1) {
			if ((val & HX_INSPECT_ELPWUG_IDLE_RAW) == HX_INSPECT_ELPWUG_IDLE_RAW)
				ret += snprintf(temp_buf + ret, len - ret, "  Check  LPWUG IDLE RAW Fail\n");
			else
				ret += snprintf(temp_buf + ret, len - ret, "  Check  LPWUG IDLE RAW OK\n");
		}
		if (g_inspt_crtra_flag[IDX_LPWUG_IDLE_NOISE_MIN] == 1 && g_inspt_crtra_flag[IDX_LPWUG_IDLE_NOISE_MAX] == 1) {
			if ((val & HX_INSPECT_ELPWUG_IDLE_NOISE) == HX_INSPECT_ELPWUG_IDLE_NOISE)
				ret += snprintf(temp_buf + ret, len - ret, "  Check  LPWUG IDLE NOISE Fail\n");
			else
				ret += snprintf(temp_buf + ret, len - ret, "  Check  LPWUG IDLE NOISE OK\n");
		}
END_FUNC:
...
static ssize_t  ctp_open_proc_read(struct file *file, char *buf,
									size_t len, loff_t *pos)
{
	int val = 0x00;
	size_t ret = 0;
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (g_inspt_crtra_flag[IDX_OPENMIN] == 1 && g_inspt_crtra_flag[IDX_OPENMAX] == 1) {
			if ((val & HX_INSPECT_EOPEN) == HX_INSPECT_EOPEN)
			{
				E("  Check Sensor open Fail\n");
				reval++;
			}else{
				I("  Check Sensor open OK\n");
				}
		}
		if (g_inspt_crtra_flag[IDX_M_OPENMIN] == 1 && g_inspt_crtra_flag[IDX_M_OPENMAX] == 1) {
			if ((val & HX_INSPECT_EMOPEN) == HX_INSPECT_EMOPEN)
			{
				E("  Check Sensor micro open Fail\n");
				reval++;
			}else{
				I("  Check Sensor micro open OK\n");
			}
		}
		if (g_inspt_crtra_flag[IDX_SHORTMIN] == 1 && g_inspt_crtra_flag[IDX_SHORTMAX] == 1) {
			if ((val & HX_INSPECT_ESHORT) == HX_INSPECT_ESHORT)
			{
				E("  Check Sensor short Fail\n");
				reval++;
			}else{
				I("  Check Sensor short OK\n");
			}
		}
END_FUNC:
		private_ts->in_self_test = 0;
		if(reval > 0){
                     result = 0; //add by songbinbo.wt for Coverity Scan CID 90604  20190522
		}else{
		    result = 1; //add by songbinbo.wt for Coverity Scan CID 90604  20190522
		}
...
/*	input_set_abs_params(ts->input_dev, ABS_MT_AMPLITUDE, 0, ((ts->pdata->abs_pressure_max << 16) | ts->pdata->abs_width_max), 0, 0);
	...
	input_set_abs_params(ts->hx_pen_dev, ABS_TILT_X, -128, 128, 0, 0);
	input_set_abs_params(ts->hx_pen_dev, ABS_TILT_Y, -128, 128, 0, 0);
...
int himax_report_data_init(void)
{
	...
	if (g_target_report_data == NULL) {
		...
		if (g_target_report_data->p_hover == NULL)
			goto mem_alloc_fail;
		g_target_report_data->p_tilt = kzalloc(sizeof(int)*2, GFP_KERNEL);
		if (g_target_report_data->p_tilt == NULL)
			goto mem_alloc_fail;
...
mem_alloc_fail:
	...
	kfree(g_target_report_data->p_tilt);
...
int himax_parse_report_points(struct himax_ts_data *ts, int ts_path, int ts_status)
{
	int x = 0, y = 0, w = 0;
#if defined(HX_PEN_FUNC_EN)
	uint8_t p_hover = 0, p_tilt = 0, p_btn = 0, p_btn2 = 0;
	...
	p_tilt = hx_touch_data->hx_coord_buf[base + 6];
	...
	if (g_ts_dbg != 0) {
		...
		D("%s: p_tilt=%d, p_hover=%d\n", __func__, p_tilt, p_hover);
...
static void himax_finger_report(struct himax_ts_data *ts)
{
	...
	if (valid) {/*Pen down*/
		...
		input_report_abs(ts->hx_pen_dev, ABS_TILT_X, 100);
		input_report_abs(ts->hx_pen_dev, ABS_TILT_Y, 100);
...
static int get_FW_version_from_bin(void)
{
        I(" %s in\n", __func__);
...
static void get_fw_ver_bin(void *device_data)
{
	...
	get_FW_version_from_bin();

	 I("himax Fw version from bin  is  0x%02x\n", g_i_FW_VER_BIN);
	 snprintf(buff, sizeof(buff) ,"HX83112A,VND:Lide,FW:0x%02x", g_i_FW_VER_BIN);
...
static void get_fw_ver_ic(void *device_data)
{
	...
	g_core_fp.fp_read_FW_ver();
	I("himax Fw version from IC  is  0x%02x\n", ic_data->vendor_touch_cfg_ver);

        snprintf(buff, sizeof(buff) ,"HX83112A,VND:Lide,FW:0x%02x", ic_data->vendor_touch_cfg_ver);
...
static struct sec_cmd himax_commands[] = {
...
#ifdef LCD_TP_RESUME_WORK
	INIT_WORK(&ts->resume_work_queue,himax_resume_workqueue);
...
#ifdef SEC_TSP_FACTORY_TEST
		val = sec_cmd_init(&private_ts->sec, himax_commands,ARRAY_SIZE(himax_commands), SEC_CLASS_DEVT_TSP);
		if (val < 0) {
