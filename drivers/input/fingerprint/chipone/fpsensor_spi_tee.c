...
int fpsensor_spidev_dts_init(fpsensor_data_t *fpsensor)
{
    ...
    if (node) {
        ...
        fpsensor->fp_rst_low = pinctrl_lookup_state(fpsensor->pinctrl1, "fpsensor_fpc_rst_low");
        ...
        fpsensor->fp_rst_high = pinctrl_lookup_state(fpsensor->pinctrl1, "fpsensor_fpc_rst_high");
...
static long fpsensor_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FPSENSOR_IOC_INIT:
        ...
        retval = request_threaded_irq(23, fpsensor_irq, NULL,
...
static int chipone_read_sensor_id(void)
{
    ...
    char writebuf[16] = {0};
	 ...
	 printk("fpsensor FINGERPRINT CHIPID is 0 \n");
    return -1;
}
...
#endif
{
    ...
    fpsensor_dev->spi               = spi_fingerprint ;
    fpsensor_dev->device_available  = 0;
