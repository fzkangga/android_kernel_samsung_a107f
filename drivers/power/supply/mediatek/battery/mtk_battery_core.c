...
void fgauge_get_profile_id(void)
{
	...
	if (ret != 0)
		bm_err("id_volt read fail\n");
	else
		bm_err("id_volt = %d\n",id_volt);

	if ((sizeof(g_battery_id_voltage) /
		sizeof(int)) != TOTAL_BATTERY_NUMBER) {
		bm_err("error! voltage range incorrect!\n");
		return;
	}

//+bug 436170  modify getian.wt 20190329  Battery ID compatible
	for (id = 0; id < TOTAL_BATTERY_NUMBER; id++) {
        ...
        //gm.battery_id = BATTERY_PROFILE_ID; //if battery is ready, please delete this line
	bm_err("Battery id (%d)\n",gm.battery_id);
//-bug 436170  modify getian.wt 20190329  Battery ID compatible
...
//+bug 436504  modify getian.wt 20190402 Optimize battery ID compatibility
