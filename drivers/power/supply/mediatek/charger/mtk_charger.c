...
//++bug432470 guojunbo.wt,add,20190313 add a node for aging test
...
int charger_psy_event(struct notifier_block *nb, unsigned long event, void *v)
{
	...
	int ret;
	int tmp = 0;
	int ret_device_file=0;//bug432470 guojunbo.wt,add,20190313 add a node for aging test
	...
	if (strcmp(psy->desc->name, "battery") == 0) {
		...
		if(node_flag == 0) {
			...
			ret_device_file = device_create_file(&psy->dev, &dev_attr_charging_enable);
			node_flag = 1;
		}
		//--bug432470 guojunbo.wt,add,20190313 add a node for aging test
	}
...
static void ato_charger_limit_soc(struct charger_manager *info, int min, int max)
{
	...
	if((limit_soc >= max) && (swchgalg->state != CHR_ERROR)) {
		_mtk_charger_do_charging(info, 0);
		chr_err("ato_charger_limit_soc:disable charging\n");
	}

    if((limit_soc <= min) && (swchgalg->state != CHR_CC)){
...
//+bug 436584  modify getian.wt 20190402 In the ATO version, the charge status is enabled after power on (no charger inserted)
				...
				struct device *dev)
{
	...
	//+bug 434514  modify getian.wt 20190321 disable battery temperature protect
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	g_info = info;//bug432470 guojunbo.wt,add,20190313 add a node for aging test
