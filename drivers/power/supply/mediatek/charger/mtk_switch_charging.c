...
//+Bug454407,zhaosidong.wt,MODIFY,20190628,if retail mode is turned on,charing current should be limited
		if(-1 != store_mode_current){
			pdata->input_current_limit = store_mode_current;
			pdata->charging_current_limit = store_mode_current;
			pr_err("store_mode enable, set charging current 650mA\n");
			goto done;
		} else {
			pdata->input_current_limit =
				info->data.ac_charger_input_current;
...
//-Bug454407,zhaosidong.wt,MODIFY,20190628,if retail mode is turned on,charing current should be limited
	...
	if (info->enable_sw_jeita) {
		...
		else if ((info->sw_jeita.cc != 0)&&(info->chr_type == STANDARD_CHARGER)) {
